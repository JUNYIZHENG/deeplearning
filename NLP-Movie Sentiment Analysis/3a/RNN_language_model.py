# A language model gives some probability distribution over the words in a sequence. We can essentially feed sequences
# into a recurrent neural network and train the model to predict the following word.
...
class StatefulLSTM(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.h is None:
            ...
            self.c = torch.zeros(state_size).to(self.device)
            self.h = torch.zeros(state_size).to(self.device)
...
class RNN_language_model(nn.Module):
    ...
    def forward(self, x, train=True):
        ...
        for i in range(no_of_timesteps):
            ...
            outputs.append(h)
        # Each output of the sequence will be used separately for calculating a particular loss and all of the losses
        # within a sequence will be summed up.
